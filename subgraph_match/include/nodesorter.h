...
{
public:
	virtual vector<int> SortNodes(TempGraph* pattern) = 0;
};

class SimpleNodeSorter :NodeSorter
{
public:
	vector<int> SortNodes(TempGraph* pattern)
	{
		int i;
		vector<int> sorted;
...
private:
	...
	void AddNodeToSortedSet(TempGraph* pattern, int node, int level,
		snode_t* nodes, vector<snode_t*> &candidates, vector<int> &nodes_order);

public:
	vector<int> SortNodes(TempGraph* pattern);
	VF3NodeSorter(Graph* target);
	VF3NodeSorter(TempGraph* target);
	~VF3NodeSorter();
