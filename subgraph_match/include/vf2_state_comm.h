...
{
public:
	VF2StateComm(Graph* data, TempGraph* ag1, TempGraph* ag2, int pid = 0, bool islazy = false
	...
	bool CompareNodeAttribute(Graph* data, int n1, int n2);
	bool CompareEdgeAttribute(Graph* data, int src1, int dst1, int src2, int dst2);
	bool NextPair(Graph* data, int* pn1, int* pn2, int prev_n1 = -1, int prev_n2 = -1);
	bool IsFeasiblePair(Graph* data, int n1, int n2);
	void AddPair(Graph* data, int n1, int n2);
	bool IsGoal()
	{
		return core_len == n1;
	}
	...
	void GetCoreSet(Graph* data, vector<int>& ans);
	...
	VF2StateComm* Clone();
	void BackTrack(Graph* data);
	void AutoFillFreeNodes(Graph* data, vector<int>& ans, int level);
};
