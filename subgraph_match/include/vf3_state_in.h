...
#ifndef VF3_STATE_IN_H
#define VF3_STATE_IN_H
...
public:
	...
	VF3StateIn(TempGraph *g1, TempGraph *g2, Match* gm, int nclass,
		int* order = NULL);
	VF3StateIn(const VF3StateIn &state);
	~VF3StateIn();
	TempGraph *GetGraph1() { return g1; }
	TempGraph *GetGraph2() { return g2; }
	bool NextPair(int *pn1, int *pn2,
		int prev_n1 = -1, int prev_n2 = -1, bool firstlevel = false);
	...
	void GetCoreSet();
	bool match_in(VF3StateIn &s, int *pn, bool firstlevel);
	int match();
