...
#ifndef VF3_STATE_SELF_H
#define VF3_STATE_SELF_H
...
public:
	...
	VF3StateSelf(TempGraph *g1, TempGraph *g2, int nclass,
		int* order = NULL);
	VF3StateSelf(const VF3StateSelf &state);
	~VF3StateSelf();
	...
	void GetCoreSet(int c2[]);

	bool match_self(VF3StateSelf &s, int *pn);
	bool match();
};
