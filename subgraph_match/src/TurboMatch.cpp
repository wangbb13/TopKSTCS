...
TurboMatch::TurboMatch(int comm, vector<vector<int>*>* boundary, unordered_map<int, unordered_map<int, unordered_map<int, int>>>* oo
	...
	, Graph* gr, TempGraph* query, TempGraph* graph)
{
	core = 0;
	comm_id = comm;
	nodeArea = boundary;
...
int TurboMatch::TurboISO(TempGraph* q, TempGraph* g, bool& candup, int comm_id) //生成NEC树时未考虑结点的邻居是否完全相同，只处理了是否完全图
{
	//NECTree q_prime;
	q_prime.vList.clear();
...
int TurboMatch::TurboISODense(TempGraph* q, TempGraph* g, bool& candup, int comm_id) //生成NEC树时未考虑结点的邻居是否完全相同，只处理了是否完全图
{
...
int TurboMatch::TurboISOBound(TempGraph* q, TempGraph* g, bool& candup) //生成NEC树时未考虑结点的邻居是否完全相同，只处理了是否完全图
{
	//NECTree q_prime;
	q_prime.vList.clear();
	q_prime.isLinked.clear();
		...
		else
		{
			...
			for (const auto& gvertex : *nodes)
			{
				...
				if (explore)
				{
					...
					if (qNumVertex != 1)
					{
						SubgraphSearch(&q_prime, order, 1, M, F, &CR, count, candup);
					}
					else
					{
						GenPerm(M, &q_prime, 0, q, g, count, candup);
...
int TurboMatch::TurboISOSuper(TempGraph* q, TempGraph* g, bool& candup) //生成NEC树时未考虑结点的邻居是否完全相同，只处理了是否完全图
{
...
int TurboMatch::ChooseStartQVertex(TempGraph* q, TempGraph* g)
{
	int K = min(3, q->nodenum);
...
void TurboMatch::FindNEC(vector<vector<Vid>> *NECV, vector<Vid> *vertexList, TempGraph *q)
{
					...
					for (int j = i + 1; j < vertexList->size(); j++)
					{
						...
						for (int mm = 0; mm < sizea; mm++)
						{
							...
							for (const auto& n : (*q->outedges)[Vj])
							{
								if (n == a[mm])
									continue;
								if ((*q->outedges)[a[mm]].find(n) == (*q->outedges)[a[mm]].end())
								{
									fmm = false;
									break;
								}
...
void TurboMatch::RewriteToNECTree(TempGraph *q, int U_s, NECTree *q_prime)
{
...
void TurboMatch::BuildNECTree(TempGraph *q, int U_s, NECTree *q_prime)
{
...
// bool TurboMatch::ExploreCR(int u_prime, unordered_set <Vid> *VM, CRTree *CR, Vid v, NECTree *q_prime, TempGraph *g, TempGraph *q, bool* visited, bool& candup)
// {
...
// bool TurboMatch::ExploreCR(int u_prime, vector<Vid> *VM, CRTree *CR, Vid v, NECTree *q_prime, TempGraph *g, TempGraph *q, bool* visited, bool& candup)
// {
// 	int vertex = q_prime->NEC[u_prime][0];
...
double TurboMatch::DetermineMatchingOrder(NECTree *q_prime, CRTree *CRTree, TurboElem *order, Vid v, int product, TempGraph *q)
{
	int ENum = 0;
...
int TurboMatch::GetFirst(vector <Vid>& C, int Size, vector <int>& rank, vector <Vid>& value, int& dc, NECTree *q_prime, int pos)
{
	...
	if (old_val != -1)
	{
		...
		{
			int commNeigh = ncMap[neigh];
			if (commNeigh != -1)
			{
				n2cNum[cur][commNeigh] = 0;
				n2cNum[neigh][old_val]--;
			}
		}
				...
				for (const auto& neigh : (*q->outedges)[cur])
				{
					...
					if (commNeigh != -1)
					{
						...
						if (n2cNum[cur][commNeigh] > data->commMaxNumTag[val][commNeigh]
							|| n2cNum[neigh][val] > data->commMaxNumTag[commNeigh][val])
						{
...
bool TurboMatch::IsJoinableSuper(TempGraph *q, TempGraph *g, int* M, int qV, int gV)
{
	unordered_set<int>* og = &(*g->outedges)[gV];
	for (const auto& u : (*q->outedges)[qV])
	{		
		int v = M[u];
		if (v != -1 && level[qV] != level[u] && og->find(v) == og->end())
		{
			return false;
		}
	}
	...
	else if (storeType == 3)
	{
		...
		for (i = 0; i < (*q->nodes).size(); i++)
		{
			if (M[(*q->nodes)[i]] != -1)
			{
				...
				r.push_back(__to_string(M[(*q->nodes)[i]]));
			}
		}
...
void TurboMatch::GenPerm(int* M, NECTree *q_prime, int i, TempGraph* q, TempGraph* g, int& count, bool& candup)
{
	int qVNum = (*q->nodes).size();// getVertexNum();
								   //cut3 off
	if (i == q_prime->vertexNum || storeType == 2 || storeType == 1)
	{
	...
	else
	{
		...
		while (!NextPerm(M, &(q_prime->NEC[i]), NULL, ranks[i], candup))
		{
			GenPerm(M, q_prime, i + 1, q, g, count, candup);
			if (storeType == 3)
			{
...
void TurboMatch::SubgraphSearchDense(TempGraph *q, NECTree *q_prime, TempGraph *g, TurboElem *order, int dc
	, int* M, CRTree *CR, int& count, bool& candup)
{
	...
	while (true)
	{
		...
		if (q_prime->vertexNum == dc + 1)
		{
			GenPerm(ncMap, q_prime, 0, q, g, count, candup);
		}
...
void TurboMatch::SubgraphSearchSuper(TempGraph *q, NECTree *q_prime, TempGraph *g, TurboElem *order, int dc
	, int* M, CRTree *CR, int& count, bool& candup)
{
	bool samelevel = false;
	int u_prime = order[dc].v;
...
void TurboMatch::initIETree(NECTree& q_prime)
{
	...
	/*for (const auto& nec : q_prime.NEC)
	{
		for (const auto& n : nec)
		{
			nodeOrder[n] = index++;
		}
	}
	for (const auto& c : gm->completeSet)
	{
		int maxorder = -1;
		int val = -1;
		for (const auto& n : c.first)
		{
			if (nodeOrder[n] > maxorder)
			{
				val = n;
				maxorder = nodeOrder[n];
			}
		}

		for (const auto& c2 : c.second)
		{
			set<int>::iterator it1 = c.first.begin();
			set<int>::iterator it2 = c2.begin();
			while (it1 != c.first.end())
			{
				if (*it1 != *it2)
				{
					gm->IETree[c.first].insert(make_pair(*it1, *it2));
					//if (*it1 == val)
					//{
					//	gm->IETree[c.first].insert(*it2);
					//}
					//else if (*it2 == val)
					//{
					//	gm->IETree[c.first].insert(*it1);
					//}
					break;
				}
				it1++;
