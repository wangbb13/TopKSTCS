...
VF3NodeSorter::VF3NodeSorter(Graph* target)
{
	probability = new SubIsoNodeProbability(target);
}

VF3NodeSorter::VF3NodeSorter(TempGraph* target)
{
	probability = new SubIsoNodeProbability(target);
}
...
vector<int> VF3NodeSorter::SortNodes(TempGraph* pattern)
{
	int nodeCount;
	int i;
...
void VF3NodeSorter::AddNodeToSortedSet(TempGraph* pattern,
