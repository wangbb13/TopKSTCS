...
void NodeProbabilityStrategy::evaluateProbabilities(Graph* source) {
	int i;
	int node_count;
	int attr;
	...
	node_count = source->node_ids.size();
	...
	for (i = 0; i < node_count; i++) {
		...
		int in_deg = source->inNetWorkSet[id].size();// InEdgeCount(i);
...
void NodeProbabilityStrategy::evaluateProbabilities(TempGraph* source) {
	int i;
	int node_count;
	int attr;
...
double SubIsoNodeProbability::getProbability(TempGraph* g, int id) {
	double node_p;
