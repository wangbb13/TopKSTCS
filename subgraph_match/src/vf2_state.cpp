...
VF2State::VF2State(Graph* data, int nodenum, vector<int>& n, unordered_map<int, unordered_map<string, string>>& na
	...
	, unordered_map<int, unordered_set<int>>& ie)
{
	...
	for (const auto& n : data->node_ids)
	{
		(*core_2)[n] = -1;
		(*in_2)[n] = 0;
		(*out_2)[n] = 0;
...
bool VF2State::CompareNodeAttribute(Graph* data, int n1, int n2)
{
	return true;
...
bool VF2State::CompareEdgeAttribute(Graph* data, int src1, int dst1, int src2, int dst2)
{
	return true;
...
bool VF2State::NextPair(Graph* data, int *pn1, int *pn2, int prev_n1, int prev_n2)
{
	if (prev_n1 == -1)
		prev_n1 = 0;
	...
	{
		while (prev_n2 < n2 &&
			((*core_2)[data->node_ids[prev_n2]] != -1 || (*out_2)[data->node_ids[prev_n2]] == 0
				|| (*in_2)[data->node_ids[prev_n2]] == 0))
		{
			prev_n2++;
		}
	}
	...
	{
		while (prev_n2 < n2 &&
			((*core_2)[data->node_ids[prev_n2]] != -1 || (*in_2)[data->node_ids[prev_n2]] == 0))
		{
			prev_n2++;
		}
	}
	...
	{
		while (prev_n2 < n2 && (*core_2)[data->node_ids[prev_n2]] != -1)
		{
			prev_n2++;
		}
	}
...
bool VF2State::IsFeasiblePair(Graph* data, int node1, int node2)
{
	assert(node1 < n1);
	assert(node2 < n2);

	node1 = (*nodes)[node1];
	node2 = data->node_ids[node2];
	...
	for (const auto& innode : data->inNetWorkSet[node2])
	{
		other2 = innode;
		if ((*core_2).find(other2) == (*core_2).end())
		{
...
void VF2State::AddPair(Graph* data, int node1, int node2)
{
	assert(node1 < n1);
	assert(node2 < n2);
	assert(core_len < n1);
...
void VF2State::BackTrack(Graph* data)
{
