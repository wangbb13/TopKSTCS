...
VF2StateComm::VF2StateComm(Graph* data, TempGraph* ag1, TempGraph* ag2, int pid, bool islazy
...
bool VF2StateComm::CompareNodeAttribute(Graph* data, int n1, int n2)
{
	return true;
	string n2_str = __to_string(n2);
...
bool VF2StateComm::CompareEdgeAttribute(Graph* data, int src1, int dst1, int src2, int dst2)
{
	return true;
	...
	string estr = "\'" + __to_string(src2) + "_" + __to_string(dst2) + "\'";
...
bool VF2StateComm::NextPair(Graph* data, int *pn1, int *pn2, int prev_n1, int prev_n2)
{
	if (prev_n1 == -1)
		prev_n1 = 0;
...
bool VF2StateComm::IsFeasiblePair(Graph* data, int node1, int node2)
{
	...
	if (active)
	{
		unordered_set<int>* outedges1;
		unordered_set<int>* inedges1;

		unordered_set<int>* outedges2 = &data->netWorkSet[node2];
		unordered_set<int>* inedges2 = &data->inNetWorkSet[node2];
		...
		for (int i = 0; i < (*g1->nodes).size(); i++)
		{
			if ((*core_1)[(*g1->nodes)[i]] != -1)
			{
					...
					&& (*oomessage)[(*g1->nodes)[i]].find(node1) != (*oomessage)[(*g1->nodes)[i]].end())
				{
					...
					for (const auto& o1 : *outedges1)
					{
						if ((*inedges2).find(o1) != (*inedges2).end())
						{
							if (data->inMemory)
							{
								int c = data->nodeInfo[o1][0];
					...
					&& (*oomessage)[node1].find((*g1->nodes)[i]) != (*oomessage)[node1].end())
				{
					...
					{
						if ((*inedges1).find(o2) != (*inedges1).end())
						{
							if (data->inMemory)
							{
								int c = data->nodeInfo[o2][0];
					...
					&& (*oimessage)[(*g1->nodes)[i]].find(node1) != (*oimessage)[(*g1->nodes)[i]].end())
				{
					...
					for (const auto& o1 : *outedges1)
					{
						if ((*outedges2).find(o1) != (*outedges2).end())
						{
							if (data->inMemory)
							{
								int c = data->nodeInfo[o1][0];
	...
	else
	{
		...
		unordered_set<int>* outedges = &data->netWorkSet[node2];
		unordered_set<int>* inedges = &data->inNetWorkSet[node2];
...
void VF2StateComm::AddPair(Graph* data, int node1, int node2)
{
	if (!degree || (*nodeArea).find((*g1->nodes)[node1]) == (*nodeArea).end())
	{
		assert(node1 < n1);
...
void VF2StateComm::GetCoreSet(vector<string>& ans)
{
	...
	for (int i = 0; i < n1; i++)
	{
		s += __to_string((*core_1)[(*g1->nodes)[i]]) + " ";
	}
...
void VF2StateComm::GetCoreSet(Graph* data, vector<int>& ans)
{
...
void VF2StateComm::BackTrack(Graph* data)
{
...
void VF2StateComm::AutoFillFreeNodes(Graph* data, vector<int>& ans, int level)
{
	...
	for (const auto n : *core_1)
	{
		...
		int rid;
		unordered_set<int>* inedges = NULL;
		unordered_set<int>* outedges = NULL;
		...
		if (g1->HasEdge(node_id, n.first))
		{
			...
			inedges = &data->inNetWorkSet[n.second];
			...
			if (area.empty())
			{
				for (const auto& node : *inedges)
				{
					if (data->inMemory)
					{
						if (data->nodeInfo[node][0] == comm_id
							&& (*core_2)[node] == -1)
						{
							area.push_back(node);
						}
