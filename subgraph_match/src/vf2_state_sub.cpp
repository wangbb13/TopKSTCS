...
VF2StateSub::VF2StateSub(TempGraph* ag1, TempGraph* ag2, bool deg, unordered_map<int, vector<int>>* boundary)
{
	g1 = ag1;
	g2 = ag2;
...
void VF2StateSub::GetCoreSet(string& ans, unordered_map<int, int>& mapper, bool iswrite)
{
	...
	for (i = 0; i < n1; i++)
	{
		if ((*core_1)[(*g1->nodes)[i]] != -1)
		{
			...
			r.push_back(__to_string((*core_1)[(*g1->nodes)[i]]));
		}
	}
