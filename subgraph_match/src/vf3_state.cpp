...
VF3State::VF3State(TempGraph *ag1, Graph *ag2, Match* m, int nclass, int* order)
{
...
bool VF3State::NextPair(int *pn1, int *pn2, int prev_n1, int prev_n2)
{
	...
	if (predecessors[curr_n1] != -1)
	{
		...
		{
		case NODE_DIR_IN:
			pred_set_size = g2->inNetWorkSet[pred_pair].size();// g2->InEdgeCount(pred_pair);

			while (last_candidate_index < pred_set_size)
			{
				prev_n2 = g2->inNetWorkVec[pred_pair][last_candidate_index];
	...
	else
	{
		...
		{
			prev_n2 = g2->node_ids[last_candidate_index];
			if (core_2[prev_n2] == -1 && g2->nodeInfo[prev_n2][1] == c)
				break;
			++last_candidate_index;
		}
