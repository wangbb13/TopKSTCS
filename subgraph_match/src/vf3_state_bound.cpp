...
/*void VF3StateBound::print_terminal(int c) {
...
void VF3StateBound::init()
{
	core_len = orig_core_len = 0;
	t2both_len = t2in_len = t2out_len = 0;
	added_node1 = -1;

	memset(termin1, 0, sizeof(int) * n1);
	memset(termout1, 0, sizeof(int) * n1);
	memset(new1, 0, sizeof(int) * n1);
	memset(core_len_c, 0, sizeof(int) * classes_count);
	memset(t2both_len_c, 0, sizeof(int) * classes_count);
	memset(t2in_len_c, 0, sizeof(int) * classes_count);
	memset(t2out_len_c, 0, sizeof(int) * classes_count);
	for (int i = 0; i <= n1; i++)
	{
		if (i<n1) {
			memset(termin1_c[i], 0, sizeof(int) * classes_count);
			memset(termout1_c[i], 0, sizeof(int) * classes_count);
			memset(new1_c[i], 0, sizeof(int) * classes_count);
		}
		core_1[i] = -1;
		memset(t1both_len_c[i], 0, sizeof(int) * classes_count);
		memset(t1in_len_c[i], 0, sizeof(int) * classes_count);
		memset(t1out_len_c[i], 0, sizeof(int) * classes_count);
	}
...
VF3StateBound::VF3StateBound(TempGraph *ag1, TempGraph *ag2, Match* m, int nclass, int* order)
{
...
* Copy constructor.
---------------------------------------------------------*/
VF3StateBound::VF3StateBound(const VF3StateBound &state)
{
	g1 = state.g1;
	g2 = state.g2;
	n1 = state.n1;
	...
	distributeplan = state.distributeplan;
	data = state.data;
	vvans = state.vvans;
...
void VF3StateBound::UpdateTerminalSetSize(int node, int level, bool* in_1, bool* out_1, bool* inserted) {
	int i, neigh, c_neigh;
...
void VF3StateBound::ComputeFirstGraphTraversing() {
...
bool VF3StateBound::NextPair(int *pn1, int *pn2, int prev_n1, int prev_n2)
{
	int curr_n1;
	...
	if (predecessors[curr_n1] != -1)
	{
		...
		{
		case NODE_DIR_IN:
			if ((*nodeArea)[curr_n1] != NULL)
			{
				pred_set_size = (*nodeArea)[curr_n1]->size();// (*g2->inedges)[pred_pair].size();// g2->InEdgeCount(pred_pair);
				unordered_set<int>* ins = &(*g2->inedges)[pred_pair];
				vector<int>* nc = (*nodeArea)[curr_n1];

				while (last_candidate_index < pred_set_size)
				{
					prev_n2 = (*nc)[last_candidate_index];//(*g2->invec)[pred_pair][last_candidate_index];
					if (core_2[prev_n2] != -1 || g2->nodeLabels[prev_n2] != c || ins->find(prev_n2) == ins->end())
			...
			{
				int comm = distributeplan[curr_n1];
				unordered_set<int>* ins = &(*g2->inedges)[pred_pair];
				vector<int>* cn = &data->communityNodes[comm];
				pred_set_size = cn->size();// g2->InEdgeCount(pred_pair);

				while (last_candidate_index < pred_set_size)
				{
					prev_n2 = (*cn)[last_candidate_index];
					if (core_2[prev_n2] != -1 || g2->nodeLabels[prev_n2] != c || ins->find(prev_n2) == ins->end())
						last_candidate_index++;
					else
						break;
		...
		case NODE_DIR_OUT:
			...
			{
				pred_set_size = (*nodeArea)[curr_n1]->size();// (*g2->inedges)[pred_pair].size();// g2->InEdgeCount(pred_pair);
				unordered_set<int>* outs = &(*g2->outedges)[pred_pair];
				vector<int>* nc = (*nodeArea)[curr_n1];

				while (last_candidate_index < pred_set_size)
				{
					prev_n2 = (*nc)[last_candidate_index];//(*g2->invec)[pred_pair][last_candidate_index];
					if (core_2[prev_n2] != -1 || g2->nodeLabels[prev_n2] != c || outs->find(prev_n2) == outs->end())
						last_candidate_index++;
					else
						break;
				}
	...
	else
	{
		...
		if ((*nodeArea)[curr_n1] != NULL)
		{
			vector<int>* nc = (*nodeArea)[curr_n1];
			int size = nc->size();
			while (last_candidate_index < size)
			{
				prev_n2 = (*nc)[last_candidate_index];
				if (core_2[prev_n2] == -1 && g2->nodeLabels[prev_n2] == c)
					break;
				++last_candidate_index;
			}

			if (last_candidate_index >= size)
				return false;
		}
		else
		{
...
--------------------------------------------------------------*/
bool VF3StateBound::IsFeasiblePair(int node1, int node2)
{
	//std::cout << "\nIF: " << node1 << " " << node2;
	...
	if (n1insize > (*g2->inedges)[node2].size()
...
-------------------------------------------------------------*/
void VF3StateBound::AddPair(int node1, int node2)
{
...
--------------------------------------------------------------*/
void VF3StateBound::GetCoreSet()
{
	vector<int> ans;
	for (int i = 0; i < n1; i++)
	{
		ans.push_back(core_1[i + 1]);
	}
	(*vvans).push_back(ans);
	//gm->logger << endl;
}
...
bool VF3StateBound::match_bound(VF3StateBound &s, int *pn)
{
	if (s.IsGoal())
	{
		++*pn;
...
static bool cmpD(const cpair& a, const cpair& b)
{
	return a.bsize < b.bsize;
}

int VF3StateBound::match()
{
	...
	{
		if ((*nodeArea)[node] == NULL)
		{
			cpair s;
			s.id = node;
			s.bsize = (*communityGraphs)[distributeplan[node]]->nodenum;
			temp.push_back(s);
		}
		else
		{
			cpair s;
			s.id = node;
			s.bsize = (*nodeArea)[node]->size();
			temp.push_back(s);
		}
	}
	sort(temp.begin(), temp.end(), cmpD);
	vector<int> result;
