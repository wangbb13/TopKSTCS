...
void VF3StateIn::print_terminal(int c) {
...
VF3StateIn::VF3StateIn(TempGraph *ag1, TempGraph *ag2, Match* m, int nclass, int* order)
{
...
* Copy constructor.
---------------------------------------------------------*/
VF3StateIn::VF3StateIn(const VF3StateIn &state)
{
	g1 = state.g1;
	g2 = state.g2;
	n1 = state.n1;
...
void VF3StateIn::UpdateTerminalSetSize(int node, int level, bool* in_1, bool* out_1, bool* inserted) {
	int i, neigh, c_neigh;
...
void VF3StateIn::ComputeFirstGraphTraversing() {
...
bool VF3StateIn::NextPair(int *pn1, int *pn2, int prev_n1, int prev_n2, bool firstlevel)
{
	int curr_n1;
		...
		case NODE_DIR_IN:
			...
			while (last_candidate_index < pred_set_size)
			{
				...
				else
				{
					if (firstlevel)
					{
						int mapped = g2->idMapping[prev_n2];
						if (gm->equalInData.find(mapped) == gm->equalInData.end())
							last_candidate_index++;
						else
						{
							memberNum = gm->equalInData[mapped].size();
							break;
						}
					}
					else
...
--------------------------------------------------------------*/
bool VF3StateIn::IsFeasiblePair(int node1, int node2)
{
	//std::cout << "\nIF: " << node1 << " " << node2;
...
-------------------------------------------------------------*/
void VF3StateIn::AddPair(int node1, int node2)
{
...
bool VF3StateIn::match_in(VF3StateIn &s, int *pn, bool firstlevel)
{
	if (s.IsGoal())
	{
		*pn += memberNum;
	...
	while (s.NextPair(&n1, &n2, n1, n2, firstlevel))
	{
		if (s.IsFeasiblePair(n1, n2))
		{
			VF3StateIn s1(s);
			s1.AddPair(n1, n2);
			if (match_in(s1, pn, false))
			{
				return true;
			}
		}
