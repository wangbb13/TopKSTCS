...
/*----------------------------------------------------------
* VF3StateSelf::VF3StateSelf(g1, g2)
* Constructor. Makes an empty state.
---------------------------------------------------------*/

VF3StateSelf::VF3StateSelf(TempGraph *ag1, TempGraph *ag2, int nclass, int* order)
{
	//assert(class_1 != NULL && class_2 != NULL);

	//VF3StateSelf::instance_count = 1;
	g1 = ag1;
	g2 = ag2;
	n1 = g1->nodenum;
	n2 = g2->nodenum;
...
/*----------------------------------------------------------
* VF3StateSelf::VF3StateSelf(state)
* Copy constructor.
---------------------------------------------------------*/
VF3StateSelf::VF3StateSelf(const VF3StateSelf &state)
{
	g1 = state.g1;
	g2 = state.g2;
	n1 = state.n1;
...
void VF3StateSelf::UpdateTerminalSetSize(int node, int level, bool* in_1, bool* out_1, bool* inserted) {
	int i, neigh, c_neigh;
...
void VF3StateSelf::ComputeFirstGraphTraversing() {
...
bool VF3StateSelf::NextPair(int *pn1, int *pn2, int prev_n1, int prev_n2)
{
	int curr_n1;
	...
	if (predecessors[curr_n1] != -1)
	{
		...
		case NODE_DIR_IN:
			...
			{
				pred_set_size = (*g2->inedges)[pred_pair].size();// g2->InEdgeCount(pred_pair);
...
/*---------------------------------------------------------------
* bool VF3StateSelf::IsFeasiblePair(node1, node2)
...
--------------------------------------------------------------*/
bool VF3StateSelf::IsFeasiblePair(int node1, int node2)
{
	//std::cout << "\nIF: " << node1 << " " << node2;
...
/*--------------------------------------------------------------
* void VF3StateSelf::AddPair(node1, node2)
* Adds a pair to the Core set of the state.
* Precondition: the pair must be feasible
-------------------------------------------------------------*/
void VF3StateSelf::AddPair(int node1, int node2)
{
...
bool VF3StateSelf::match_self(VF3StateSelf &s, int *pn)
{
	if (s.IsGoal())
	{
		//int n = s.CoreLen();
...
bool VF3StateSelf::match()
{
	...
	this->order = resort.data();
	ComputeFirstGraphTraversing();

	int n = 0;
	return match_self(*this, &n);
